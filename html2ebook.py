...
def print_usage():
    print(
        'html2ebook.py -i <input_dir> -o <output_file> [--help] [--index <index_page>] [--identifier <identifier>] [--title <title>] [--author <author>] [--cover <cover>] [--sort]')
...
def get_mime_type(file_name):
    res = mime_types.guess_type(file_name)[0]
    return res if res != None else "application/octet-stream"
...
def get_html_title_info(html_file):
    ...
    if root.find("body") == None:  # Skip some invalid
        return (None, None)
...
def main(argv):
    ...
    opts, _ = getopt(argv, 'i:o:', ["input=", "output=", "index=",
                     "identifier=", "help", "sort", "author=", "title=", "cover="])
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            ...
            relative_path = os.path.relpath(
                abs_path, input_dir).replace('\\', '/')

            if file == index_page and not has_index:
                index_item, page_title = add_html_to_book(
                    book, abs_path, relative_path)
                index_link = epub.Link(
                    'index.html', title=page_title, uid='index')
            ...
            if file.endswith('.html'):
                ...
                item = add_assest_to_book(
                    book, abs_path, relative_path, 'text/css')
                book.add_item(item)
            else:
    ...
    if sort_pages:
        def get_index(file_name): return os.path.split(
            file_name)[1].split('.')[0]

        digit_items = [i for i in html_items if get_index(
            i.file_name).isdigit()]
        non_digit_items = [
            i for i in html_items if not get_index(i.file_name).isdigit()]
